...
function activityLabel(action: string): string {
  return action === 'governance-request-submitted' ? 'Request Submitted' : humanize(action);
}
...
export function GovernanceDashboard({ onNavigate }: { onNavigate: (screen: string) => void }) {
  ...
  const metricCards = [
    ...
    ['Legacy FA Account Requests', data?.metrics.adminAccountRequests, UserCog, 'text-blue-700 bg-blue-50'],
  ...
  const controls = [
    ['No self-approval', data ? data.controls.noSelfApproval : null, ShieldCheck],
    ['Audit trail active', data ? data.controls.auditTrailActive : null, FileCheck2],
    [
      'Audit history read-only',
      data ? !data.controls.auditHistoryMutableByUsers : null,
      CheckCircle2,
    ],
  ] as const;
  ...
  return (
    <div className="flex-1 overflow-y-auto bg-[#f7f8fa]">
      <SuperAdminHeader title="Approval Center" />
            ...
            <div className="mt-6 grid gap-6 xl:grid-cols-[minmax(0,1.7fr)_minmax(320px,0.8fr)]">
                      ...
                      {!data && (
                        <tr>
                          <td colSpan={5} className="px-6 py-12 text-center text-gray-500">
                            Approval information is temporarily unavailable.
                          </td>
                        </tr>
                      )}
                      {data && data.priorityRequests.length === 0 && (
              ...
              <Panel className="p-6">
                ...
                <div className="mt-6 space-y-5">
                  ...
                  {data && data.recentActivity.length === 0 && <p className="text-sm text-gray-500">No governance activity yet.</p>}
                  {data?.recentActivity.map((event) => (
                    <div key={event.id} className="relative flex gap-3 pl-1">
                      ...
                      <div className="min-w-0">
                        <p className="text-sm font-semibold text-gray-900">{activityLabel(event.action)}</p>
                ...
                {controls.map(([label, ok, Icon]) => {
                  ...
                  return (
                    <div key={String(label)} className="flex items-center gap-3 border-r border-gray-100 last:border-r-0">
                      ...
                      <div>
                        <p className="text-sm font-semibold text-gray-900">{String(label)}</p>
                        <p className={`text-xs ${ok === null ? 'text-gray-500' : 'text-emerald-700'}`}>
                          {ok === null ? 'Temporarily unavailable' : ok ? 'Operating' : 'Review required'}
                        </p>
                      </div>
                    </div>
                  );
