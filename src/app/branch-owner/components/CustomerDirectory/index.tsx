...
import { Badge } from '../../../branch-manager/components/Badge';
import { Progress } from '../../../branch-manager/components/Progress';
...
import '../../../branch-manager/bm-theme.css';
 ...
 */
export default function Customers(_props: CustomersProps = {}) {
    ...
    const { data: customersData, error: queryError, isLoading, refetch: loadRequests } = useQuery({
        queryKey: ['customers', selectedBranchId, debouncedSearch],
        queryFn: () => {
            if (!selectedBranchId) return { customers: [] };
            const trimmed = debouncedSearch.trim();
            const path = trimmed.length >= 2 ? `/customers?search=${encodeURIComponent(trimmed)}&branchId=${selectedBranchId}` : `/customers?branchId=${selectedBranchId}`;
    ...
    return (
        <div className="flex-1 overflow-y-auto bm-root">
            <div className="pt-2">
                    ...
                    <table className={styles.table}>
                        <thead>
                            <tr>
                                ...
                                <th style={{ whiteSpace: 'nowrap', minWidth: '200px' }}>Delivery Address</th>
                                <th>Source</th>
                                <th>Last Order</th>
                                <th>Registered</th>
                                <th></th>
                                ...
                                >
                                    ...
                                    <td>
                                        <div className={styles.nameCell}>
                                            ...
                                            <span className={styles.boldText} style={{ whiteSpace: 'nowrap' }}>{c.name}</span>
                                        </div>
                                    </td>
                                    <td style={{ whiteSpace: 'nowrap' }}>{formatPHMobile(c.contact_number)}</td>
                                    <td className={styles.mutedText} style={{ minWidth: '200px' }}>{c.delivery_address}</td>
                                    <td style={{ whiteSpace: 'nowrap' }}>
                                        ...
                                        {c.registration_source === 'staff-created' ? (
                                            <Badge variant="secondary" style={{ fontWeight: 500 }}>Staff-created</Badge>
                                        ) : (
                                            <Badge variant="info" style={{ fontWeight: 500 }}>Self-registered</Badge>
                                        )}
                                    </td>
