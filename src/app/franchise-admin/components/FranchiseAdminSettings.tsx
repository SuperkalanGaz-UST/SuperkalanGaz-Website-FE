...
function initials(firstName: string, lastName: string): string {
  return `${firstName.charAt(0)}${lastName.charAt(0)}`.toUpperCase() || 'FA';
}
...
/** Franchise Administrator self-service profile. All persistence goes through the NestJS API. */
export function FranchiseAdminSettings() {
  ...
  const handleProfileSubmit = async (event: FormEvent<HTMLFormElement>) => {
    ...
    const canonicalPhone = form.mobile.trim() ? toE164PhMobile(form.mobile) : null;
    setSaving(true);
    try {
      const response = await apiFetch('/users/me', {
        method: 'PATCH',
            ...
            <form onSubmit={handleProfileSubmit} noValidate>
              <div className="grid gap-10 lg:grid-cols-[220px_minmax(0,1fr)]">
                ...
                <section aria-labelledby="personal-details-heading">
                  <h2 id="personal-details-heading" className="text-xl font-semibold text-gray-900">
                    Personal details
                  </h2>
